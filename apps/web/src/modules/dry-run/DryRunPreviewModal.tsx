...
export function DryRunPreviewModal({
  ...
  return (
    <div className="ops-drawer-backdrop" role="presentation" onClick={() => !busy && onCancel()}>
      <div
        className="ops-drawer ops-dryrun-modal"
        role="dialog"
        aria-modal="true"
        aria-labelledby="ops-dryrun-title"
        onClick={(ev) => ev.stopPropagation()}
      >
        <header className="ops-drawer-header">
          <h2 id="ops-dryrun-title">{title}</h2>
          <button type="button" className="ops-btn" disabled={busy} onClick={onCancel}>
            Close
          </button>
        </header>
        {!summary ? (
          ...
          <>
                      ...
                      <tr key={`${e.index}-${e.entityType}-${e.entityId ?? "x"}`}>
                        ...
                        <td>
                          <div className="ops-dryrun-diff">
                            <div>
                              <span className="ops-muted">before</span>{" "}
                              <code>{jsonPreview(e.before ?? null)}</code>
                            </div>
                            <div>
                              <span className="ops-muted">after</span>{" "}
                              <code>{jsonPreview(e.after ?? null)}</code>
                            </div>
            ...
            <div className="ops-drawer-row-actions">
              ...
              <button type="button" className="ops-btn ops-btn-primary" disabled={busy || !summary} onClick={onConfirm}>
