...
const email = "session-inact@example.com";
...
describe("session inactivity expiry (Epic 7)", () => {
  ...
  it("rejects token past sliding inactivity exp while still within absolute window", async () => {
    const user = await prisma.user.findUniqueOrThrow({ where: { email } });
    const issuedMs = Date.now() - 70 * 60 * 1000;
    const { token } = createSessionToken(user.id, "operator", issuedMs);
