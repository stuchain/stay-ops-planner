...
## Seed production DB via GitHub Actions

**Purpose (EPICS S4):** idempotently upsert bootstrap **admin** and **father** operator accounts against the production Neon database using the same `packages/db/prisma/seed.ts` as local CI.

1. On GitHub Environment **`production`**, add the same secrets you use for **[Migrate production DB](../../.github/workflows/migrate-production.yml)** (`DATABASE_URL`), plus bootstrap values for **Seed production DB**:
   - **Admin:** `BOOTSTRAP_ADMIN_PASSWORD` (must be an **Environment secret**). `BOOTSTRAP_ADMIN_EMAIL` may be an **Environment secret** or an **Environment variable** named `BOOTSTRAP_ADMIN_EMAIL` (the workflow passes `vars` first, then `secrets`).
   - **Father:** `BOOTSTRAP_FATHER_PASSWORD` (secret). `BOOTSTRAP_FATHER_EMAIL` — secret or variable, same pattern as admin.
   - Optional secret: `BOOTSTRAP_FATHER_ROLE` (`viewer` \| `operator` \| `admin`; default `operator`)
2. Actions → **[Seed production DB](../../.github/workflows/seed-production.yml)** → **Run workflow** (manual `workflow_dispatch` only).

The workflow runs `pnpm --filter @stay-ops/db seed` with those env vars. Concurrency group `seed-production` avoids overlapping runs canceling each other. **Do not** paste passwords into logs; GitHub masks configured secrets.

**Re-run behavior:** seed always sets `password_hash` from the current env values. After operators rotate passwords in production, avoid re-running seed unless you intend to reset credentials from secrets.

You can still run `pnpm --filter @stay-ops/db seed` from a trusted machine with the same env vars and `DATABASE_URL` instead.
...
| Schema + users | On a trusted machine, with **`DATABASE_URL`** set to **Neon**: `pnpm --filter @stay-ops/db migrate:deploy` then `pnpm --filter @stay-ops/db seed` (set **`BOOTSTRAP_ADMIN_*`** and **`BOOTSTRAP_FATHER_*`** for seed), or use Actions **[Seed production DB](../../.github/workflows/seed-production.yml)**. Never commit secrets. |
...
4. Run seed bootstrap for operator accounts (when identity is not yet provisioned), with **`BOOTSTRAP_ADMIN_*`** and **`BOOTSTRAP_FATHER_*`** set:
   - `pnpm --filter @stay-ops/db seed`, **or** GitHub Actions **[Seed production DB](../../.github/workflows/seed-production.yml)**
