...
- Production DB (EPICS S2): manual **[Migrate production DB](.github/workflows/migrate-production.yml)** (`workflow_dispatch`, GitHub Environment `production` + `DATABASE_URL`) and optional **[Verify production readiness](.github/workflows/verify-production-readiness.yml)** for ad-hoc `/api/health/ready` checks — see [docs/runbooks/production-deploy.md](docs/runbooks/production-deploy.md).
