...
- **Browser E2E (Playwright)**: `apps/web/tests/e2e/` — desktop Chromium and mobile viewport (390×844). Flows cover login/shell, **Unassigned queue assign** and **PATCH reassign conflict** (same APIs as calendar drag; see [docs/phases/phase-06-calendar-ux.md](docs/phases/phase-06-calendar-ux.md) “E2E automation vs product behavior”), maintenance blocks (create/edit/delete/overlap), mobile quick assign, and cleaning lifecycle. One-time browsers: `pnpm --filter @stay-ops/web test:e2e:install`. After `seed`, run **`pnpm --filter @stay-ops/db seed:e2e`** for fixtures. Set `E2E_ADMIN_EMAIL` / `E2E_ADMIN_PASSWORD` to match `BOOTSTRAP_ADMIN_*`. Run: `pnpm --filter @stay-ops/web test:e2e`. CI: [`.github/workflows/e2e.yml`](.github/workflows/e2e.yml). See [docs/runbooks/local-dev.md](docs/runbooks/local-dev.md).
